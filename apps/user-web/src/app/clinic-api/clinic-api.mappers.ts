...
export function mapPatientPrescriptionFromApi(row: Record<string, unknown>): Prescription {
  return {
    ...
    method:
      ((row['methodOption'] as Record<string, unknown> | null)?.['label'] as string | null) ?? null,
    diagnosedDisease:
      ((row['diagnosedDiseaseOption'] as Record<string, unknown> | null)?.['label'] as
        string | null) ?? null,
...
export function mapConsultationFromApi(row: Record<string, unknown>): Consultation {
  ...
  return {
    ...
    consultationMode:
      (row['consultationMode'] as Consultation['consultationMode']) || 'CLINIC_QUEUE',
    ...
    payment: row['payment']
      ? {
          ...
          refundedAmountInPaise:
            ((row['payment'] as Record<string, unknown>)['refundedAmountInPaise'] as
              number | undefined) ?? 0,
          status: (row['payment'] as Record<string, unknown>)['status'] as Payment['status'],
          billingPlanCode:
            ((row['payment'] as Record<string, unknown>)['billingPlanCode'] as string | null) ||
            null,
          lineItems:
            ((row['payment'] as Record<string, unknown>)['lineItems'] as Record<
              string,
              unknown
            > | null) || null,
          providerOrderId:
            ((row['payment'] as Record<string, unknown>)['providerOrderId'] as string | null) ||
            null,
        }
      : null,

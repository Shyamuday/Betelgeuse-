...
export function registerHrDoctorRoutes(router: Router) {
  ...
  router.put(
    ...
    asyncRoute(async (req, res) => {
      ...
      const nextDoctorType =
        (doctorType as HomeopathicDoctorType | undefined) ?? existing.doctorType;
      const nextFocus =
        specialtyFocus === null
          ? null
      ...
      const updated = await prisma.doctor.update({
        ...
        data: {
          ...hrFields,
          ...(nextDoctorType === HomeopathicDoctorType.PSYCHOLOGIST
  ...
  router.post(
    HR_API_ROUTES.DOCTOR_LETTER,
    hrAuthMiddleware,
    asyncRoute(async (req, res) => {
      const id = req.params['id'] as string;
      const { clinicName, clinicAddress } = req.body as {
        clinicName?: string;
  ...
  router.get(
    ...
    hrAuthMiddleware,
    asyncRoute(async (req, res) => {
      const letter = await prisma.joiningLetter.findUnique({
        where: { doctorUserId: req.params['id'] as string }
      });
      if (!letter) {
        res.status(404).json({ error: 'Letter not yet generated' });
        return;
      }
      res.json({ letter });
    })
  ...
  router.put(
    HR_API_ROUTES.DOCTOR_ASSIGNMENT,
    hrAuthMiddleware,
    asyncRoute(async (req, res) => {
      const { storeIds } = getAccess(req);
      const id = req.params['id'] as string;
      const { isOnline, clinicStoreId } = req.body as {
        isOnline?: boolean;
        clinicStoreId?: string | null;
      };

      if (clinicStoreId && storeIds && !storeIds.includes(clinicStoreId)) {
        res.status(403).json({ error: 'You do not have access to that store' });
        return;
      }
